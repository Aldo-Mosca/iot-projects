...
This project uses ESP-IDF with the Matter SDK. Key tools: `idf.py` for building and flashing, `chip-tool` for commissioning. When build errors occur, check Python virtual environment activation, API version mismatches between ESP-IDF versions, and directory paths for spaces or special characters.
...
When debugging ESP-IDF or embedded firmware issues, always consider volatile qualifiers, register caching, and RTOS task context when dealing with global/shared variables.
...
When users report build or runtime errors, always ask for the full error output if not provided. Do not guess at the error — request the paste first.
